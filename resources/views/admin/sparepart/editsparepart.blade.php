...
<div class="flex items-center mt-20 mb-2 ml-4"> <!-- Tambahkan mb-2 untuk mengurangi jarak bawah -->
    <a href="{{ route('sparepart.show', ['id' => $sparepart->id]) }}" class="flex items-center text-gray-700 hover:text-gray-900">
    ...
    <div class="bg-white p-6 rounded-lg shadow-lg">
        ...
        <form action="{{ route('sparepart.update', $sparepart->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div>
                ...
                <input
                    type="text" id="Nomor Sparepart" name="nomor_sparepart" value="{{ $sparepart->nomor_sparepart }}"
            ...
            <div>
                ...
                <input
                    type="text" id="Nama Sparepart" name="nama_sparepart" value="{{ $sparepart->nama_sparepart }}"
                    class="mt-1 block w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2  text-sm focus:ring-[#6C757D] focus:border-[#6C757D] text-[#6C757D]"
            ...
            <div>
                ...
                <input
                    type="text" id="Jumlah" name="jumlah" value="{{ $sparepart->jumlah }}"
                    class="mt-1 block w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2  text-sm focus:ring-[#6C757D] focus:border-[#6C757D] text-[#6C757D]"
                    placeholder="Jumlah ">
            </div>
            ...
            <div>
                ...
                <textarea
                    id="Keterangan" name="keterangan" value="{{ $sparepart->keterangan }}"
                    class="mt-1 block w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2  text-sm focus:ring-[#6C757D] focus:border-[#6C757D] text-[#6C757D]"
                    placeholder="Tambahkan keterangan tambahan..." rows="3">{{ $sparepart->keterangan ?? '' }}</textarea>
            </div>
                ...
                <div class="mt-1 relative w-full">
                    ...
                    <div class="border border-gray-300 rounded-lg flex items-center justify-between p-2 w-full">
                        <span id="file-name" class="text-gray-400 text-sm pl-2">
                            @if($sparepart->foto)
                            {{ basename($sparepart->foto) }} <!-- Tampilkan nama file yang sudah ada -->
                            @else
