...
<div class="flex items-center mt-20 mb-2 ml-4"> <!-- Tambahkan mb-2 untuk mengurangi jarak bawah -->
    <a href="{{ route('sparepart.index') }}" class="flex items-center text-gray-700 hover:text-gray-900">
        ...
        <span class="text-lg font-semibold">Tambah Sparepart</span>
    </a>
</div>
...
<main class="p-6 -mt-4 max-w-full mx-auto"> <!-- mt-4 untuk mendekatkan ke tombol back -->
    <div class="bg-white p-6 rounded-lg shadow-lg">
        <!-- 🔹 Tulisan "Edit APAR" -->
        <h2 class="text-center text-xl font-bold mb-2">Tambah Data Sparepart</h2>
        <p class="text-center text-gray-500 mb-4">Masukkan data Sparepart dengan tepat</p>

        <!-- Form -->
        <form class="space-y-4" action="{{ route('sparepart.store') }}" method="POST" enctype="multipart/form-data">
            @csrf
            <!-- NIP Input -->
            <div>
                ...
                <input
                    type="text" id="Nomor Sparepart" name="nomor_sparepart"
            ...
            <div>
                ...
                <input
                    type="text" id="Nama Sparepart" name="nama_sparepart"
                    class="mt-1 block w-full border border-gray-300 rounded-lg p-2 focus:outline-none focus:ring-2  text-sm focus:ring-[#6C757D] focus:border-[#6C757D] text-[#6C757D]"
