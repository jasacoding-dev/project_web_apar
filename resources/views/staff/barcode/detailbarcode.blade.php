...
<div class="flex items-center mt-20 mb-2 ml-4"> <!-- Tambahkan mb-2 untuk mengurangi jarak bawah -->
    <a href="{{ route('staff.scan.barcode') }}" class="flex items-center text-gray-700 hover:text-gray-900">
...
<main class="p-6 -mt-4 max-w-full mx-auto "> <!-- mt-4 untuk mendekatkan ke tombol back -->
        ...
        <div class="flex flex-col md:flex-row gap-4">
            ...
            <div class="pr-8 md:pr-16 text-sm flex-1">
                <div class="mb-4">
                    ...
                    <p>{{ $barcode->apar->nomor_apar }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Merek Alat</p>
                    <p>{{ $barcode->apar->merek }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Jenis Media</p>
                    <p>{{ $barcode->apar->jenisMedia->nama_media }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Sistem Kerja Alat</p>
                    <p>{{ $barcode->apar->sistem_kerja }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Lokasi Penempatan</p>
                    <p>{{ $barcode->lokasi->nama_ruangan }}</p>
                </div>
            </div>
            ...
            <div class="pl-0 md:pl-10 text-sm mr-48 flex-1">
                <div class="mb-4">
                    ...
                    <p>{{ $barcode->lokasi->nama_gedung }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Model Tabung</p>
                    <p>{{ $barcode->apar->modelTabung->model_tabung }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Kapasitas</p>
                    <p>{{ $barcode->apar->kapasitas }}</p>
                </div>
                <div class="mb-4">
                    <p class="font-bold">Satuan Kerja </p>
                    <p>{{ $barcode->lokasi->satuan_kerja }}</p>
                </div>
                <div class="mb-4">
        ...
        <div class="flex justify-center items-center mt-6">
            ...
            <div id="statusModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
                <div class="bg-white p-6 rounded-lg w-96 shadow-lg relative">
                    ...
                    <form id="statusForm" action="{{ route('barcode.updateStatus', $barcode->apar->nomor_apar) }}" method="POST">
                        @csrf
                        <div class="space-y-2">
                            <label class="flex items-center border p-2 rounded border-gray-400 cursor-pointer">
                                <input type="radio" name="status" value="Baik" class="hidden" onclick="toggleCheckbox(this)">
                            ...
                            <label class="flex items-center border p-2 rounded border-gray-400 cursor-pointer">
                                <input type="radio" name="status" value="Perlu Perbaikan" class="hidden" onclick="toggleCheckbox(this)">
                                <span class="w-5 h-5 border-2 border-gray-400 flex items-center justify-center rounded mr-2 transition-all">
                                    <svg class="hidden w-4 h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 00-1.414 0L7 13.586l-2.293-2.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l9-9a1 1 0 000-1.414z" clip-rule="evenodd" />
                                    </svg>
                            ...
                            <label class="flex items-center border p-2 rounded border-gray-400 cursor-pointer">
                                <input type="radio" name="status" value="Refilling" class="hidden" onclick="toggleCheckbox(this)">
                                <span class="w-5 h-5 border-2 border-gray-400 flex items-center justify-center rounded mr-2 transition-all">
                                    <svg class="hidden w-4 h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 00-1.414 0L7 13.586l-2.293-2.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l9-9a1 1 0 000-1.414z" clip-rule="evenodd" />
                                    </svg>
                            ...
                            <label class="flex items-center border p-2 rounded border-gray-400 cursor-pointer">
                                <input type="radio" name="status" value="Kustom" class="hidden" onclick="toggleCheckbox(this)">
                                <span class="w-5 h-5 border-2 border-gray-400 flex items-center justify-center rounded mr-2 transition-all">
                                    <svg class="hidden w-4 h-4 text-white" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                                        <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 00-1.414 0L7 13.586l-2.293-2.293a1 1 0 00-1.414 1.414l3 3a1 1 0 001.414 0l9-9a1 1 0 000-1.414z" clip-rule="evenodd" />
                                    </svg>
                        ...
                        <div class="flex justify-center">
                            <button type="submit" class="mt-4 w-48 bg-[#FFDF00] text-black font-bold py-2 px-4 rounded">
                                Simpan
                            </button>
                        </div>
                    </form>
    ...
    <script>
        ...
        function handleDelete1() {
            console.log("Tombol Ya di Modal Sukses diklik");
            ...
            let selectedText = selectedRadio.value;
            ...
            setTimeout(() => {
                if (selectedText === "Baik") {
                    ...
                    window.location.href = "{{ route('staff.status.perbaikan', $barcode->apar->nomor_apar) }}";
                } else if (selectedText === "Kustom") {
                    window.location.href = "{{ route('staff.status.kustom', $barcode->apar->nomor_apar) }}";
                }
            }, 1000);
        }
        ...
        document.getElementById('statusForm').addEventListener('submit', function(event) {
            ...
            if (selectedRadio && selectedRadio.value === "Refilling") {
