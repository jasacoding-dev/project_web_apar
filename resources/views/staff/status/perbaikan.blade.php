...
<div class="flex items-center mt-20 mb-2 ml-4">
    <a href="{{ url()->previous() }}" class="flex items-center text-gray-700 hover:text-gray-900">
        <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
        </svg>
        <span class="text-lg font-semibold">Daftar Sparepart</span>
    </a>
</div>

<!-- Form Container -->
<main class="p-6 -mt-4 max-w-full mx-auto">
    <div class="bg-white shadow-md rounded-b-lg p-4 w-auto sm:w-[96%] md:w-full min-h-[96vh] md:min-h-full flex flex-col justify-start overflow-auto">
        ...
        <div class="flex flex-row items-center justify-between w-full space-x-4 mb-4">
            ...
            <div class="flex items-center bg-white px-4 py-2 rounded-full w-full sm:max-w-sm border border-black">
                ...
                <input type="text" id="searchSparepart" placeholder="Cari sparepart..." class="bg-transparent outline-none w-full ml-2 text-sm text-gray-600">
            </div>
        </div>

        <form id="sparepartForm" action="{{ route('staff.status.perbaikan.store', $nomor_apar) }}" method="POST">
            @csrf
                ...
                <table class="w-full border-collapse border border-gray-300">
                    <thead>
                        <tr class="bg-[#0168AD] text-white">
                            <th class="border border-gray-300 px-4 py-2 text-center w-[50px]"></th>
                    ...
                    <tbody id="sparepartTableBody">
                        ...
                        <tr class="bg-white">
                            <td class="border border-gray-300 px-4 py-2 text-center">
                                <input type="checkbox" name="spareparts[]" value="{{ $sparepart->id }}" class="row-checkbox w-4 h-4">
                            </td>
                            <td class="border border-gray-300 px-4 py-2 font-bold">{{ $sparepart->nama_sparepart }}</td>
                        </tr>
                        @endforeach
                    </tbody>
                </table>
            ...
            <div class="flex justify-center mt-40">
                <button type="button" id="nextButton" class="bg-[#FFDF00] text-black font-bold px-6 py-2 rounded-lg">
                    Selanjutnya
                </button>
            </div>
        </form>
        ...
        <div id="deleteModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
            <div class="bg-white rounded-lg shadow-lg p-6 w-96 text-center relative">
                ...
                <h2 class="text-lg font-semibold mb-2">Apakah ingin mengajukan permohonan perbaikan?</h2>
        ...
        <div id="successModal" class="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 hidden z-50">
            <div class="bg-white rounded-lg shadow-lg p-6 w-96 text-center relative">
                ...
                <p>Permohonan perbaikan berhasil diajukan</p>

                <div class="mt-4 flex justify-center gap-4">
                    <button id="deleteButton" class="bg-[#FFDF00] text-black font-semibold px-4 py-1 w-40 rounded-lg" onclick="submitForm()">Kembali</button>
                </div>
            </div>
        </div>
        ...
        <script>
            ...
            function submitForm() {
                document.getElementById("sparepartForm").submit();
            }
            ...
            document.getElementById("searchSparepart").addEventListener("input", function() {
                ...
                let rows = document.querySelectorAll("#sparepartTableBody tr");

                rows.forEach(row => {
                    let sparepartName = row.querySelector("td:nth-child(2)").textContent.toLowerCase();
                    if (sparepartName.includes(searchQuery)) {
                        row.style.display = "";
                    } else {
